...
const CommercialBuy = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        console.log('Available categories:', [...new Set(res.data.map(p => p.category))]);
        console.log('Available property types:', [...new Set(res.data.map(p => p.propertyType))]);
        
        // Try multiple approaches to find commercial properties
        // 1. Check if there are properties with 'Commercial for Buy' category
        let categoryFiltered = res.data.filter(property => property.category === 'Commercial for Buy');
        console.log('Properties with Commercial for Buy category:', categoryFiltered);
        
        // 2. If no results, try filtering by 'Buy' category and commercial property types
        if (categoryFiltered.length === 0) {
          const commercialTypes = ['Office', 'Retail', 'Warehouse', 'Industrial', 'Shop', 'Showroom', 'Commercial'];
          categoryFiltered = res.data.filter(property => 
            property.category === 'Buy' && 
            (property.propertyType === 'Commercial' || commercialTypes.includes(property.propertyType) || 
             (property.commercialType && commercialTypes.includes(property.commercialType)))
          );
          console.log('Properties with Buy category and commercial types:', categoryFiltered);
        }
  ...
  const handleMapViewClick = () => {
    navigate('/map-view/commercial-buy');
  };
